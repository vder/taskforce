...
trait Circe extends DerivedCirceCodec with instances.Circe {
  ...
  implicit lazy val decodeOperator: Decoder[Operator] =
    Decoder.decodeString.emap { x =>
      helpers.operatorFromString.lift(x).toRight(s"Invalid operator:$x")
    }
  ...
  implicit lazy val decodeStatus: Decoder[Status] =
    Decoder.decodeString.emap { x =>
      helpers.statusFromString.lift(x).toRight(s"Invalid status:$x")
    }
  ...
  implicit lazy val decodeIn: Decoder[Criteria.In] = deriveDecoder

  implicit lazy val encodeIn: Encoder[Criteria.In] = deriveEncoder
