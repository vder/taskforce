...
object ResponseError {
  ...
  def fromAppError: PartialFunction[Throwable, ResponseError] = {
    ...
    case AppError.DuplicateProjectName(projectName) =>
      DuplicateProjectName(s"project name '$projectName' already exists")
  ...
  final case class DuplicateProjectName(message: String) extends ResponseError
  ...
  implicit class ResponseErrorOps1[F[_]: MonadThrow, A](fa: F[Either[ResponseError, A]]) {
    def extractFromEffectandMerge: F[Either[ResponseError, A]] =
